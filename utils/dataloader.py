...
# change
# def draw_gaussian1(heatmap,center,rw,rh,k=1):
#     diameterw = 2* rw+1
#     diameterh = 2* rh+1
#     gaussian = gaussian2D((diameterh,diameterw),sigmah=diameterh/6,sigmaw=diameterw/6)
...
#     masked_headmap = heatmap[y-top:y+bottom,rw-left:rw+right]
#     masked_gaussian= gaussian[rh-top:rh+bottom,rw-left:rw+right]

#     if min(masked_gaussian.shape) > 0 and min(masked_headmap.shape) > 0:
#         np.maximum(masked_headmap,masked_gaussian*k,out=masked_headmap)
#     return heatmap

# def gaussian2D(shape, sigmah=1,sigmaw=1):
#     m, n = [(ss - 1.) / 2. for ss in shape]
#     y, x = np.ogrid[-m:m+1,-n:n+1]
#     #h = np.exp(-(x * x + y * y) / (2 * sigma * sigma))
#     h = np.exp(-(x * x ) / (2 * sigmah/3 * sigmah/3)-(y * y) / (2 * sigmaw/3 * sigmaw/3))
...
def gaussian_radius(det_size,min_overlap=0.7):
    ...
    b = min((1 - min_overlap) / (1 + min_overlap) * height, 0.7 * height)
...
def draw_gaussian1(heatmap, center, h_radius, w_radius, k=1):
...
class CenternetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(box)):
            ...
            if h > 0 and w > 0:
                radius, rb = gaussian_radius((math.ceil(h), math.ceil(w)))
                ...
                #   绘制高斯热力图
                #----------------------------#
                batch_hm[:, :, cls_id] = draw_gaussian1(batch_hm[:, :, cls_id], ct_int, radius,rb)
                #---------------------------------------------------#
                #   计算宽高真实值
                #---------------------------------------------------#
